...
const customerDropDown = { component: 'dropdown', name: 'accountId', label: 'Customer', apiURL: '/api/accounts?accountType=Customer' };
    ...
    invoiceCreationHandler = () => {
        if (!this.state.updateMode) {
            globalService.post('/api/invoices', this.state.values).then(res => {
                this.responseHandler('Invoice Creted Successfully', 'success');
                this.props.history.goBack();
            }).catch(err => {
                this.responseHandler('Invalid Invoice', 'error');
            });
        } else {
            globalService.put('/api/invoices', this.state.values).then(res => {
                this.responseHandler('Invoice Updated Successfully', 'success');
                this.props.history.goBack();
            }).catch(err => {
                this.responseHandler('Invalid Invoice', 'error');
            });
        ...
        return (
            <div className={classes.root}>
                <div className={classes.content}>
                    ...
                    <Card elevation={0}>
                        <CardHeader subheader="The invoice creation form" title="Invoice" />
                        <Divider />
                        <CardContent>
                            <Grid container spacing={3}>
                                ...
                                <Grid item md={4} xs={4}>
                                    <TextField name="date" variant="outlined" margin="dense" fullWidth size="small" label="Invoice Date" type="date"
...
export default withRouter(withStyles(useStyles)(Invoice));